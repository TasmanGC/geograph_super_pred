...
from ..imports.import_classes import CoreNode
import pandas as pd

def construct_standard_nodes(graph,mode='het'):
    core_node_list = []
    counter = 0
    if mode=='het':
        for ntype in ['A','B']:
            nloc = graph.ndata['nloc'][ntype].numpy()
            labl = graph.ndata['labl'][ntype].numpy()
            data = graph.ndata['data'][ntype].numpy()
            
            for i,loc in enumerate(nloc):
                core_node  = CoreNode(loc,params={'labl':labl[i],'type':ntype,'data':data[i]})
                core_node_list.append(core_node)
                counter = counter+1
    if mode!='het':
        nloc = graph.ndata['nloc'].numpy()
        labl = graph.ndata['labl'].numpy()
        data = graph.ndata['data'].numpy()
            
        for i,loc in enumerate(nloc):
            core_node  = CoreNode(loc[0],params={'labl':labl[i],'type':mode,'data':data[i]})
            core_node_list.append(core_node)
            counter = counter+1

    return(core_node_list)
    
def construct_standard_edges(graph,mode='het'):
